...
{
    game_t *game = getGame();
    float len = (float)game->window->mode.width;
    float dist = sfView_getCenter(game->camera).x * (float)(1.0f - parallax_spd);
    ...
    sfVector2f new_pos = sfRectangleShape_getPosition(layer);
    sfRectangleShape_setPosition(layer, (sfVector2f) {new_pos.x + dist, new_pos.y});
